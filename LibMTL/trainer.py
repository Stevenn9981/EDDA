...
from util_functions import test_model
...
class Trainer(nn.Module):
    ...
    def train(self, train_dataloaders, test_dataloaders, epochs,
        ...
        for epoch in range(epochs):
            ...
            for batch_index in range(train_batch):
                ...
                if not self.multi_input:
                    ...
                    for tn, task in enumerate(self.task_name):
                        ...
                        train_pos = self.model(train_input, task)[task]
                        train_neg = self.model(train_gt, task)[task]
                        train_losses[tn] = self._compute_loss(train_pos, train_neg, task)
                        self.meter.update(0, 1, task)
            ...
            if self.meter.best_result['epoch'] == epoch:
                if not os.path.exists('model'):
                    os.makedirs('model')
                torch.save({'recModel': self.state_dict(), 'results': self.meter.best_result},
                           'model/recModel_{}_{}.pth'.format('_'.join(sorted(self.task_name)),
                                                             os.getpid()))
            if self.scheduler is not None:
                self.scheduler.step()

            if epoch - self.meter.best_result['epoch'] > 10:
                break
    ...
    def test(self, test_dataloaders, epoch=None, mode='test'):
        ...
        with torch.no_grad():
            if not self.multi_input:
                ...
                for tn, task in enumerate(self.task_name):
                    if mode == 'val':
                        auc, recall1, recall3 = test_model(self.model, TRAIN_RECORDS[task], VAL_RECORDS[task], task)
                    else:
                        auc, recall1, recall3 = test_model(self.model, TRAIN_RECORDS[task], TEST_RECORDS[task], task)
                    self.meter.update(auc, 1, task)
                    recall_dict[task] = {1: recall1, 3: recall3}
