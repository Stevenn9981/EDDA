...
class _PerformanceMeter(object):
    def __init__(self, task_dict, multi_input, base_result=None):
        ...
        self.best_result = {'improvement': 0, 'epoch': 0, 'result': 0, 'sep_dom': {}}
    ...
    def get_score(self, mode):
        with torch.no_grad():
            for tn, task in enumerate(self.task_name):
                ...
                self.loss_item[tn] = self.losses[task]._average_loss() if mode == 'train' else 0
    ...
    def display(self, mode, epoch):
        ...
        for tn, task in enumerate(self.task_name):
            ...
            if mode == 'train':
                sum_m += self.loss_item[tn]
            else:
                sum_m += self.results[task][0] * self.weight[task][0]
            print('| ', end='')
        print('AVG: {:.4f} | '.format(sum_m / len(self.task_name)), end='')
        print('Time: {:.4f}'.format(self.end_time - self.beg_time), end='')
        print(' | ') if mode != 'test' else print(' | Best Epoch: {} |\n'.format(self.best_result['epoch']))

    def display_best_result(self):
        print('=' * 40)
        print('Best Result: Epoch {}, result {}'.format(self.best_result['epoch'], self.best_result['result']))
        print('Best Result of each domain: {}'.format(self.best_result['sep_dom']))
        print('=' * 40)
    ...
    def _update_best_result(self, new_result, epoch):
        ...
        if improvement > self.best_result['improvement']:
            ...
            self.best_result['epoch'] = epoch
            self.best_result['result'] = new_result
        for domain in new_result:
            if domain not in self.best_result['sep_dom']:
                self.best_result['sep_dom'][domain] = new_result[domain][0]
            elif self.best_result['sep_dom'][domain] < new_result[domain][0]:
                self.best_result['sep_dom'][domain] = new_result[domain][0]
